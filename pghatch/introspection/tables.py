...
class PgSQLImplementationInfo(SQLModel, table=True):
    ...
    implementation_info_id: str = Field(
        sa_column=Column("implementation_info_id", String, primary_key=True)
    )
    implementation_info_name: str = Field(
        sa_column=Column("implementation_info_name", String)
    )
    integer_value: Optional[int] = Field(
        default=None, sa_column=Column("integer_value", Integer)
    )
    character_value: Optional[str] = Field(
        default=None, sa_column=Column("character_value", String)
    )
    comments: str = Field(sa_column=Column("comments", String))
...
class PgAggregate(SQLModel, table=True):
    ...
    agginitval: Optional[str] = Field(
        default=None, sa_column=Column("agginitval", Text)
    )
    aggminitval: Optional[str] = Field(
        default=None, sa_column=Column("aggminitval", Text)
    )
...
class PgAmop(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "amopopr", "amoppurpose", "amopfamily", name="pg_amop_opr_fam_index"
        ),
        {"schema": "pg_catalog"},
    )
...
class PgAttribute(SQLModel, table=True):
    ...
    __table_args__ = (
        PrimaryKeyConstraint(
            "attrelid", "attnum", name="pg_attribute_relid_attnum_index"
        ),
    ...
    attstattarget: Optional[int] = Field(
        default=None, sa_column=Column("attstattarget", SmallInteger)
    )
    attacl: Optional[Any] = Field(default=None, sa_column=Column("attacl", NullType))
    attoptions: Optional[list] = Field(
        default=None, sa_column=Column("attoptions", ARRAY(Text()))
    )
    attfdwoptions: Optional[list] = Field(
        default=None, sa_column=Column("attfdwoptions", ARRAY(Text()))
    )
    attmissingval: Optional[Any] = Field(
        default=None, sa_column=Column("attmissingval", NullType)
    )

    def get_class(self, introspection: "Introspection") -> Optional["PgClass"]:
        return next(
            filter(lambda x: x.relnamespace == self.attrelid, introspection.classes)
        )
    ...
    def get_description(self, introspection: "Introspection") -> Optional[str]:
        return introspection.get_description(
            introspection.PG_CLASS, self.attrelid, self.attnum
        )

    def get_tags_and_description(
        self, introspection: "Introspection"
    ) -> Tuple[list, Optional[str]]:
        tags = introspection.get_tags(
            introspection.PG_CLASS, self.attrelid, self.attnum
        )
        description = introspection.get_description(
            introspection.PG_CLASS, self.attrelid, self.attnum
        )
...
class PgAuthMembers(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "member", "roleid", "grantor", name="pg_auth_members_member_role_index"
        ),
        UniqueConstraint(
            "roleid", "member", "grantor", name="pg_auth_members_role_member_index"
        ),
...
class PgAuthid(SQLModel, table=True):
    ...
    rolpassword: Optional[str] = Field(
        default=None, sa_column=Column("rolpassword", Text)
    )
    rolvaliduntil: Optional[datetime] = Field(
        default=None, sa_column=Column("rolvaliduntil", DateTime(True))
    )
...
class PgCast(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "castsource", "casttarget", name="pg_cast_source_target_index"
        ),
        {"schema": "pg_catalog"},
    )
...
class PgClass(SQLModel, table=True):
    ...
    reloptions: Optional[list] = Field(
        default=None, sa_column=Column("reloptions", ARRAY(Text()))
    )
    relpartbound: Optional[Any] = Field(
        default=None, sa_column=Column("relpartbound", NullType)
    )
    ...
    def get_description(
        self, introspection: "Introspection", PG_CLASS
    ) -> Optional[str]:
    ...
    def get_acl(
        ...
        objtype = (
            OBJECT_SEQUENCE if getattr(self, "relkind", None) == "S" else OBJECT_TABLE
        )
        return parse_acls(introspection, self.relacl, self.relowner, objtype)

    def get_attribute(
        self, by, introspection: "Introspection"
    ) -> Optional["PgAttribute"]:
        ...
        if "number" in by and by["number"]:
            return next(
                (
                    ...
                    if getattr(att, "attnum", None) == by["number"]
                ),
                None,
            )
            ...
            return next(
                (
                    att
                    for att in attributes
                    if getattr(att, "attname", None) == by["name"]
                ),
                None,
            )
        return None
    ...
    def get_inherited(self, introspection: "Introspection") -> list["PgInherits"]:
        return [
            inh
            for inh in introspection.inherits
            if getattr(inh, "inhrelid", None) == self.oid
        ]
    ...
    def get_access_method(self, introspection: "Introspection") -> Optional["PgAm"]:
        if self.relam is not None:
            return next(
                (
                    am
                    for am in introspection.am
                    if getattr(am, "oid", None) == self.relam
                ),
                None,
            )
        return None
...
class PgCollation(SQLModel, table=True):
    ...
    collcollate: Optional[str] = Field(
        default=None, sa_column=Column("collcollate", Text)
    )
    collctype: Optional[str] = Field(default=None, sa_column=Column("collctype", Text))
    colllocale: Optional[str] = Field(
        default=None, sa_column=Column("colllocale", Text)
    )
    collicurules: Optional[str] = Field(
        default=None, sa_column=Column("collicurules", Text)
    )
    collversion: Optional[str] = Field(
        default=None, sa_column=Column("collversion", Text)
    )
...
class PgConstraint(SQLModel, table=True):
    ...
    conkey: Optional[list] = Field(
        default=None, sa_column=Column("conkey", ARRAY(SmallInteger()))
    )
    confkey: Optional[list] = Field(
        default=None, sa_column=Column("confkey", ARRAY(SmallInteger()))
    )
    conpfeqop: Optional[list] = Field(
        default=None, sa_column=Column("conpfeqop", ARRAY(OID()))
    )
    conppeqop: Optional[list] = Field(
        default=None, sa_column=Column("conppeqop", ARRAY(OID()))
    )
    conffeqop: Optional[list] = Field(
        default=None, sa_column=Column("conffeqop", ARRAY(OID()))
    )
    confdelsetcols: Optional[list] = Field(
        default=None, sa_column=Column("confdelsetcols", ARRAY(SmallInteger()))
    )
    conexclop: Optional[list] = Field(
        default=None, sa_column=Column("conexclop", ARRAY(OID()))
    )
    ...
    def get_tags_and_description(
        ...
        """
        return introspection.get_tags_and_description(
            introspection.PG_CONSTRAINT, self.oid
        )
...
class PgConversion(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "conname", "connamespace", name="pg_conversion_name_nsp_index"
        ),
        UniqueConstraint(
            "connamespace",
            "conforencoding",
...
class PgDatabase(SQLModel, table=True):
    ...
    daticurules: Optional[str] = Field(
        default=None, sa_column=Column("daticurules", Text)
    )
    datcollversion: Optional[str] = Field(
        default=None, sa_column=Column("datcollversion", Text)
    )
...
class PgDbRoleSetting(SQLModel, table=True):
    ...
    __table_args__ = (
        PrimaryKeyConstraint(
            "setdatabase", "setrole", name="pg_db_role_setting_databaseid_rol_index"
        ),
        {"schema": "pg_catalog"},
    )
    ...
    setconfig: Optional[list] = Field(
        default=None, sa_column=Column("setconfig", ARRAY(Text()))
    )
...
class PgDescription(SQLModel, table=True):
    ...
    __table_args__ = (
        PrimaryKeyConstraint(
            "objoid", "classoid", "objsubid", name="pg_description_o_c_o_index"
        ),
        {"schema": "pg_catalog"},
    )
...
class PgEnum(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "enumtypid", "enumsortorder", name="pg_enum_typid_sortorder_index"
        ),
        {"schema": "pg_catalog"},
    )
    ...
    def get_py_type(self, introspection: "Introspection") -> type:
        """
        Get the Python type associated with this enum.
        """
        from pghatch.introspection.pgtypes import get_py_type

        return get_py_type(
            introspection=introspection, typ=self.get_type(introspection)
        )
...
class PgEventTrigger(SQLModel, table=True):
    ...
    evttags: Optional[list] = Field(
        default=None, sa_column=Column("evttags", ARRAY(Text()))
    )
...
class PgExtension(SQLModel, table=True):
    ...
    extconfig: Optional[list] = Field(
        default=None, sa_column=Column("extconfig", ARRAY(OID()))
    )
    extcondition: Optional[list] = Field(
        default=None, sa_column=Column("extcondition", ARRAY(Text()))
    )
...
class PgForeignDataWrapper(SQLModel, table=True):
    ...
    fdwoptions: Optional[list] = Field(
        default=None, sa_column=Column("fdwoptions", ARRAY(Text()))
    )
...
class PgForeignServer(SQLModel, table=True):
    ...
    srvversion: Optional[str] = Field(
        default=None, sa_column=Column("srvversion", Text)
    )
    srvacl: Optional[Any] = Field(default=None, sa_column=Column("srvacl", NullType))
    srvoptions: Optional[list] = Field(
        default=None, sa_column=Column("srvoptions", ARRAY(Text()))
    )
...
class PgForeignTable(SQLModel, table=True):
    ...
    ftoptions: Optional[list] = Field(
        default=None, sa_column=Column("ftoptions", ARRAY(Text()))
    )
...
class PgIndex(SQLModel, table=True):
    ...
    indexprs: Optional[Any] = Field(
        default=None, sa_column=Column("indexprs", NullType)
    )
    ...
    def get_keys(self, introspection: "Introspection") -> list[Optional["PgAttribute"]]:
        ...
        return [
            None
            if key == 0
            else next((a for a in attrs if getattr(a, "attnum", None) == key), None)
            for key in keys
        ]
...
class PgInherits(SQLModel, table=True):
    ...
    __table_args__ = (
        PrimaryKeyConstraint(
            "inhrelid", "inhseqno", name="pg_inherits_relid_seqno_index"
        ),
...
class PgInitPrivs(SQLModel, table=True):
    ...
    __table_args__ = (
        PrimaryKeyConstraint(
            "objoid", "classoid", "objsubid", name="pg_init_privs_o_c_o_index"
        ),
        {"schema": "pg_catalog"},
    )
...
class PgNamespace(SQLModel, table=True):
    ...
    def get_class(self, introspection: "Introspection", by):
        return next(
            (
                child
                for child in introspection.classes
                if getattr(child, "relnamespace", None) == self.oid
                and getattr(child, "relname", None) == by.get("name")
            ),
            None,
        )
    ...
    def get_constraint(self, introspection: "Introspection", by):
        return next(
            (
                child
                for child in introspection.constraints
                if getattr(child, "connamespace", None) == self.oid
                and getattr(child, "conname", None) == by.get("name")
            ),
            None,
        )
    ...
    def get_procs(self, introspection: "Introspection", by):
        return [
            child
            for child in introspection.procs
            if getattr(child, "pronamespace", None) == self.oid
            and getattr(child, "proname", None) == by.get("name")
        ]
...
class PgOpclass(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "opcmethod", "opcname", "opcnamespace", name="pg_opclass_am_name_nsp_index"
        ),
        {"schema": "pg_catalog"},
    )
...
class PgOpfamily(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "opfmethod", "opfname", "opfnamespace", name="pg_opfamily_am_name_nsp_index"
        ),
        {"schema": "pg_catalog"},
    )
...
class PgPartitionedTable(SQLModel, table=True):
    ...
    partexprs: Optional[Any] = Field(
        default=None, sa_column=Column("partexprs", NullType)
    )
...
class PgPolicy(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "polrelid", "polname", name="pg_policy_polrelid_polname_index"
        ),
        {"schema": "pg_catalog"},
    )
    ...
    polwithcheck: Optional[Any] = Field(
        default=None, sa_column=Column("polwithcheck", NullType)
    )
...
class PgProc(SQLModel, table=True):
    ...
    proallargtypes: Optional[list] = Field(
        default=None, sa_column=Column("proallargtypes", ARRAY(OID()))
    )
    proargmodes: Optional[list] = Field(
        default=None, sa_column=Column("proargmodes", ARRAY(String()))
    )
    proargnames: Optional[list] = Field(
        default=None, sa_column=Column("proargnames", ARRAY(Text()))
    )
    proargdefaults: Optional[Any] = Field(
        default=None, sa_column=Column("proargdefaults", NullType)
    )
    protrftypes: Optional[list] = Field(
        default=None, sa_column=Column("protrftypes", ARRAY(OID()))
    )
    probin: Optional[str] = Field(default=None, sa_column=Column("probin", Text))
    prosqlbody: Optional[Any] = Field(
        default=None, sa_column=Column("prosqlbody", NullType)
    )
    proconfig: Optional[list] = Field(
        default=None, sa_column=Column("proconfig", ARRAY(Text()))
    )
    ...
    def get_tags_and_description(
        self, introspection: "Introspection", PG_PROC
    ) -> Optional[dict]:
    ...
    def get_arguments(self, introspection: "Introspection") -> list[ProcArgument]:
        ...
        for arglist in (self.proargtypes, self.proallargtypes):
            ...
            for idx, type_id in enumerate(arglist):
                ...
                is_variadic = mode == "v"
                has_default = (
                    idx >= len(arglist) - self.pronargdefaults
                    if self.proargdefaults
                    else False
                )
    ...
    def get_acl(self, introspection: "Introspection") -> Any:
        from pghatch.introspection.acl import parse_acls, OBJECT_FUNCTION
...
class PgPublicationNamespace(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "pnnspid", "pnpubid", name="pg_publication_namespace_pnnspid_pnpubid_index"
        ),
        {"schema": "pg_catalog"},
    )
...
class PgPublicationRel(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "prrelid", "prpubid", name="pg_publication_rel_prrelid_prpubid_index"
        ),
...
class PgShdepend(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            ...
            "objid",
            "objsubid",
            name="pg_shdepend_depender_unique_index",
        ),
        {"schema": "pg_catalog"},
    )
...
class PgShseclabel(SQLModel, table=True):
    ...
    __table_args__ = (
        PrimaryKeyConstraint(
            "objoid", "classoid", "provider", name="pg_shseclabel_object_index"
        ),
        {"schema": "pg_catalog"},
    )
...
class PgStatistic(SQLModel, table=True):
    ...
    staattnum: int = Field(
        sa_column=Column("staattnum", SmallInteger, primary_key=True)
    )
    ...
    stanumbers1: Optional[list] = Field(
        default=None, sa_column=Column("stanumbers1", ARRAY(REAL()))
    )
    stanumbers2: Optional[list] = Field(
        default=None, sa_column=Column("stanumbers2", ARRAY(REAL()))
    )
    stanumbers3: Optional[list] = Field(
        default=None, sa_column=Column("stanumbers3", ARRAY(REAL()))
    )
    stanumbers4: Optional[list] = Field(
        default=None, sa_column=Column("stanumbers4", ARRAY(REAL()))
    )
    stanumbers5: Optional[list] = Field(
        default=None, sa_column=Column("stanumbers5", ARRAY(REAL()))
    )
    stavalues1: Optional[Any] = Field(
        default=None, sa_column=Column("stavalues1", NullType)
    )
    stavalues2: Optional[Any] = Field(
        default=None, sa_column=Column("stavalues2", NullType)
    )
    stavalues3: Optional[Any] = Field(
        default=None, sa_column=Column("stavalues3", NullType)
    )
    stavalues4: Optional[Any] = Field(
        default=None, sa_column=Column("stavalues4", NullType)
    )
    stavalues5: Optional[Any] = Field(
        default=None, sa_column=Column("stavalues5", NullType)
    )
...
class PgStatisticExt(SQLModel, table=True):
    ...
    stxstattarget: Optional[int] = Field(
        default=None, sa_column=Column("stxstattarget", SmallInteger)
    )
    stxexprs: Optional[Any] = Field(
        default=None, sa_column=Column("stxexprs", NullType)
    )
...
class PgStatisticExtData(SQLModel, table=True):
    ...
    __table_args__ = (
        PrimaryKeyConstraint(
            "stxoid", "stxdinherit", name="pg_statistic_ext_data_stxoid_inh_index"
        ),
        {"schema": "pg_catalog"},
    )

    stxoid: Any = Field(sa_column=Column("stxoid", OID, primary_key=True))
    stxdinherit: bool = Field(
        sa_column=Column("stxdinherit", Boolean, primary_key=True)
    )
    stxdndistinct: Optional[Any] = Field(
        default=None, sa_column=Column("stxdndistinct", NullType)
    )
    stxddependencies: Optional[Any] = Field(
        default=None, sa_column=Column("stxddependencies", NullType)
    )
    stxdmcv: Optional[Any] = Field(default=None, sa_column=Column("stxdmcv", NullType))
    stxdexpr: Optional[Any] = Field(
        default=None, sa_column=Column("stxdexpr", NullType)
    )
...
class PgSubscription(SQLModel, table=True):
    ...
    subslotname: Optional[str] = Field(
        default=None, sa_column=Column("subslotname", String)
    )
...
class PgSubscriptionRel(SQLModel, table=True):
    ...
    __table_args__ = (
        PrimaryKeyConstraint(
            "srrelid", "srsubid", name="pg_subscription_rel_srrelid_srsubid_index"
        ),
        {"schema": "pg_catalog"},
    )
    ...
    srsublsn: Optional[Any] = Field(
        default=None, sa_column=Column("srsublsn", NullType)
    )
...
class PgTablespace(SQLModel, table=True):
    ...
    spcoptions: Optional[list] = Field(
        default=None, sa_column=Column("spcoptions", ARRAY(Text()))
    )
...
class PgTrigger(SQLModel, table=True):
    ...
    tgoldtable: Optional[str] = Field(
        default=None, sa_column=Column("tgoldtable", String)
    )
    tgnewtable: Optional[str] = Field(
        default=None, sa_column=Column("tgnewtable", String)
    )
...
class PgTsConfigMap(SQLModel, table=True):
    ...
    __table_args__ = (
        PrimaryKeyConstraint(
            "mapcfg", "maptokentype", "mapseqno", name="pg_ts_config_map_index"
        ),
        {"schema": "pg_catalog"},
    )

    mapcfg: Any = Field(sa_column=Column("mapcfg", OID, primary_key=True))
    maptokentype: int = Field(
        sa_column=Column("maptokentype", Integer, primary_key=True)
    )
...
class PgTsDict(SQLModel, table=True):
    ...
    dictinitoption: Optional[str] = Field(
        default=None, sa_column=Column("dictinitoption", Text)
    )
...
class PgTsTemplate(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "tmplname", "tmplnamespace", name="pg_ts_template_tmplname_index"
        ),
        {"schema": "pg_catalog"},
    )
...
class PgType(SQLModel, table=True):
    ...
    typdefaultbin: Optional[Any] = Field(
        default=None, sa_column=Column("typdefaultbin", NullType)
    )
    typdefault: Optional[str] = Field(
        default=None, sa_column=Column("typdefault", Text)
    )
    ...
    def get_py_type(self, introspection: "Introspection") -> Optional[type]:
        """
        Returns the Python type associated with this PostgreSQL type.
        This is a simplified version and may not cover all cases.
        """
        from pghatch.introspection.pgtypes import get_py_type

        return get_py_type(introspection=introspection, typ=self)
...
class PgUserMapping(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "umuser", "umserver", name="pg_user_mapping_user_server_index"
        ),
        {"schema": "pg_catalog"},
    )
    ...
    umoptions: Optional[list] = Field(
        default=None, sa_column=Column("umoptions", ARRAY(Text()))
    )
...
class PgRoles(SQLModel, table=True):
    ...
    rolsuper: Optional[bool] = Field(
        default=None, sa_column=Column("rolsuper", Boolean)
    )
    rolinherit: Optional[bool] = Field(
        default=None, sa_column=Column("rolinherit", Boolean)
    )
    rolcreaterole: Optional[bool] = Field(
        default=None, sa_column=Column("rolcreaterole", Boolean)
    )
    rolcreatedb: Optional[bool] = Field(
        default=None, sa_column=Column("rolcreatedb", Boolean)
    )
    rolcanlogin: Optional[bool] = Field(
        default=None, sa_column=Column("rolcanlogin", Boolean)
    )
    rolreplication: Optional[bool] = Field(
        default=None, sa_column=Column("rolreplication", Boolean)
    )
    rolconnlimit: Optional[int] = Field(
        default=None, sa_column=Column("rolconnlimit", Integer)
    )
    rolpassword: Optional[str] = Field(
        default=None, sa_column=Column("rolpassword", Text)
    )
    rolvaliduntil: Optional[Any] = Field(
        default=None, sa_column=Column("rolvaliduntil", NullType)
    )
    rolbypassrls: Optional[bool] = Field(
        default=None, sa_column=Column("rolbypassrls", Boolean)
    )
    rolconfig: Optional[list] = Field(
        default=None, sa_column=Column("rolconfig", ARRAY(Text()))
    )
